<template>
  <div>
    ...
    <div class="img">
      <img @click="gotoSearchAct()" src="../assets/search.png" />
    </div>
      ...
      <div class="gift_bottom">
        <van-pull-refresh v-model="refreshing" @refreshing="onRefresh">
          <van-list
            ...
            @load="onLoad"
          >
            <div class="gift_content" v-for="item in List" :key="item.actid" @click="gotoact(item.actid,item.actvip)">
  ...
  methods:{
    gotoact(actid,vip){
      ...
      if(name == null){
        ...
        if(uservip == 1 || vip == 0){
          that.$router.push("Hddetail/"+actid);
        }else{
          Toast("请先充值vip");
        }
      }
