  ...
  methods: {
    ...
    attention(tip){
      ...
      if(tip.Attention == "已关注"){
          this.axios.get("attention/del?authorid="+tip.userid+"&uid="+this.username).then(res=>{
            tip.Attention = '关注';
          })
      }else{
          this.axios.get("attention/add?authorid="+tip.userid+"&uid="+this.username).then(res=>{
            tip.Attention = '已关注';
          })
      }
    },
