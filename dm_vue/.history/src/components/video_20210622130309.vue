  ...
  <div>
    <van-tabs v-model="activeName" sticky>
      <van-tab title="关注" name="a">
        <!-- 关注 -->
        <div class="attention" v-if="attentionList.length == 0">
          <img style="width: 100%" src="../assets/nothing.png" />
        </div>

        <div v-else>
          <van-cell v-for="item in attentionList" :key="item.userid" @click="goAuthord(item)">
            <div class="userlist">
              <van-image :src="pic+item.userimg" round width="50" height="50"></van-image>
              <div class="name">
                {{item.username}}
                <div class="vip" v-if="item.uservip == 1">
                  <p>vip</p>
                </div>
              </div>
              <div class="right">
            ...
            <div class="recommend_bottom" v-for="item in recommend" :key="item.postid">
              ...
              <div class="recommend_bottom_bottom">
                <div>
                  ...
                  <span style="font-size: 10px; color: #555" @click="goodjog(item.postid,item.postgood)">{{item.postgood}}</span>
                </div>
              </div>
...
export default {
  ...
  methods: {
    goodjog(postid,postgood) {
      postgood += 1;
      this.axios.get("userpost/updateGood?postid="+postid+"&postgood="+postgood).then(res=>{})
    },
