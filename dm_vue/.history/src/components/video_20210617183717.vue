  ...
  methods: {
    ...
    attention(tip){
      ...
      if(tip.Attention == "已关注"){
          this.axios.get("attention/del?authorid="+tip.userid+"&userid="+this.username).then(res=>{
            tip.Attention = '关注';
          })
      }else{
          this.axios.get("attention/add?authorid="+tip.userid+"&userid="+this.username).then(res=>{
            tip.Attention = '已关注';
          })
      }
    },
      ...
      this.axios.get("userpost/listAll?current="+this.pageIndex+"&size=3").then((response)=>{
        ...
        let promise = new Promise(function(reslove,reject){
          for(let i=0;i<response.data.records.length;i++){
            that.axios.get("attention/exist?authorid="+response.data.records[i].userid+"&userid="+localStorage.getItem("username")).then(res=>{
              // console.log(res);
              if(res.data=="exist"){
