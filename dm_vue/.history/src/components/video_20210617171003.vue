          ...
          >
            <div class="recommend_bottom" v-for="item in recommend" :key="item.postid">
              ...
              <div class="recommend_bottom_top">
                ...
                <p @click="changeT(item.Attention,this)" class="recommend_bottom_top_right">{{item.Attention}}</p>
              </div>
  ...
  methods: {
    ...
    onLoad(){
      ...
      this.axios.get("userpost/listAll?current="+this.pageIndex+"&size=3").then((response)=>{
        ...
        for(let i=0;i<response.data.records;i++){
          console.log("11111111111");
          this.axios.get("attention/exist?authorid="+response.data.records[i].userid+"&uid="+localStorage.getItem("userid")).then(res=>{
            if(res.data=="exist"){
