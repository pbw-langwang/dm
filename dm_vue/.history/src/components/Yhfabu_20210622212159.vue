<template>
  <div>
    ...
    <div class="btn">
        ...
        <van-button type="primary" round plain @click="batchPass">批量过审</van-button>
    </div>
...
export default {
  ...
  methods:{
    ...
    pass(postid){
      this.axios.get("userpost/pass?postid="+postid).then(res=>{
        this.refreshing=true;
        this.onRefresh();
      })
    },
    ...
    batchPass(){
      ...
      this.axios.post("userpost/batchPass",{"delIds":delIds}).then(res=>{
        this.refreshing = true;
        this.onRefresh();
      });
    },
