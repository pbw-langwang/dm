  ...
  <div>
    <van-tabs v-model="activeName" sticky>
      <van-tab title="关注" name="a">
        ...
        <div v-else>
          <van-cell v-for="item in attentionList" :key="item.userid" @click="getInfo(item)">
            <div class="userlist">
              <van-image :src="picIp+item.userimg" round width="50" height="50"></van-image>
  ...
  methods: {
    ...
    getattentionList(){
      ...
      this.axios.get("attention/attentionsList?userid="+userid).then(res=>{
        // console.log(res);
        this.attentionList = this.attentionList.concat(res);
      })
    }
  },
};
