          ...
          >
            <div class="recommend_bottom" v-for="item in recommend" :key="item.postid">
              ...
              <div class="recommend_bottom_top">
                ...
                <p @click="changeT" class="recommend_bottom_top_right">关注</p>
              </div>
              ...
              <div class="recommend_bottom_bottom">
                ...
                <div @click.once="goodjog(item)">
                  <van-icon name="good-job-o" :color="item.show == true ? 'red' : '#555'" />
...
export default {
  ...
  methods: {
    changeT(e) {
      if (e.target.outerText == "关注") {
        e.target.innerText = "已关注";
        e.target.style.cssText = "background:red;color:white";
      } else {
        e.target.innerText = "关注";
        e.target.style.cssText = "background:white;color:black";
      }
    },
    ...
    onLoad(){
      ...
      this.axios.get("userpost/listAll?current="+this.pageIndex+"&size=10").then((response)=>{
