            ...
            <div class="recommend_bottom" v-for="item in recommend" :key="item.postid">
              ...
              <div class="recommend_bottom_bottom">
                <div>
                  ...
                  <span style="font-size: 10px; color: #555" @click="goodjog(item.postid,item.postsee)">{{item.postgood}}</span>
                </div>
              </div>
...
export default {
  ...
  methods: {
    goodjog(postid,postsee) {
      this.axios.get("userpost/updateGood?postid="+postid+"&postsee="+postsee).then(res=>{
        console.log(res);
      })
    },
    onLoad(){
