  ...
  export default {
    ...
    methods:{
      ...
      onSubmit(values){
        ...
        this.axios.post("usermessage/regist",formData,{'Content-Type':'multipart/form-data'}).then((response) => {
          if(response.data=="ok"){
            Toast("注册成功");
      ...
      sendMail(){
        if(this.user.email.length>0){
          this.axios.post("usermessage/sendMail?useremail="+this.user.email).then(response=>{
