        ...
        <van-pull-refresh v-model="refreshing" @refreshing="onRefresh">
          ...
          >
            <div class="recommend_bottom" v-for="item in recommend" :key="item.postid">
              <!-- 头部 -->
              <div class="recommend_bottom_top"  v-if="item.postgood == 1">
              ...
              <!-- 中间 -->
              <div  @click="gotopostdetail(item.postid)" v-if="item.postgood == 1">
...
export default {
  ...
  methods: {
    onLoad(){
      ...
      this.axios.get("userpost/listAll?current="+this.pageIndex+"&size=100").then((response)=>{
